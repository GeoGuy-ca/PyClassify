...
class MyApp(QMainWindow, Ui_MainWindow):
    ...
    def openfile(self):
        ...
        x_size = int(data_set.RasterYSize)
        y_size = int(data_set.RasterXSize)
    ...
    def k_means(self):
        ...
        stream.synchronize()
        k_means.k_means_classify[(x_size*y_size)/1024+1, 1024, stream](dev_data, dev_result, x_size, dev_clusters, num_clust, preGPU.shape[2], max_iteration)
        stream.synchronize()
        start = time.time()
        ...
        #         print result[i, j]
        test = np.empty([3, 3])
        pixmap = QImage(result, 8011, 4000, QImage.Format_Indexed8)
        pixmap.setColor(0, qRgb(255, 163, 39))
